...
* **Largest Possible Number from joining array elements**: Turn them into a string, Then using the `sort` algorithm, pass a comparison function that  appends string `B` to string `A`. Then appends string `A` to string `B`. And finally runs `str1.compare(str2) > 0 ? 1 : 0`.
